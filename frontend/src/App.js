...
function App() {
  ...
  const API_URL = "http://10.32.6.50:8001/process/";
  ...
  // Map display label → backend task name
  const getBackendTask = (displayTask) => {
    if (displayTask === "Image Upscaling") return "Super Resolution";
    return displayTask;
  };
  ...
  const applyAI = async () => {
    ...
    try {
      ...
      formData.append("task", getBackendTask(task));  // ✅ FIX: map to backend name
              ...
              <div style={styles.imageRow}>
                ...
                <div style={styles.imageContainer}>
                  {resultImage ? (
                    ...
                    !loading && <p style={{ ...styles.imageLabel, marginTop: 60 }}>Apply AI to see output →</p>
                  )}
